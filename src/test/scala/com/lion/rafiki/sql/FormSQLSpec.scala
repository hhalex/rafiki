...
import com.lion.rafiki.domain.company.{Form, FormTree}
...
object FormSQLSpec extends Specification with IOChecker {
  ...
  import FormSQL._
  import FormTreeSQL._

  val formId = Form.tag(2)
  val treeId = FormTree.tag(2)
  val treeKey = (treeId, FormTree.Kind.Group)
  ...
  check(insertTreeHeaderQ(FormTree.Kind.Group, treeKey.some))
