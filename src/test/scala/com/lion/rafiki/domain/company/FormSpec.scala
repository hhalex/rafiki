...
"""
  ...
  // Form tree encodings
  val correctlyEncodedGroup = Form.Tree
    .Group(Nil)
    .asInstanceOf[Form.Tree]
    .asJson
    .noSpaces should beEqualTo("""{"children":[]}""")
  val correctlyEncodedQuestion = Form.Tree
  ...
  val correctlyEncodedQuestionWithAnswers = Form.Tree
    .Question(
      "label",
      "text",
      List(
  ...
  val correctlyEncodedText = Form.Tree
    .Text("text")
    .asInstanceOf[Form.Tree]
    .asJson
    .noSpaces should beEqualTo("""{"text":"text"}""")

  val correctlyEncodedGroupUpdate = Form.Tree
    .GroupWithKey(Form.Tree.tag(2), Nil)
    .asInstanceOf[Form.Tree]
    .asJson
    .noSpaces should beEqualTo("""{"id":2,"children":[]}""")
  val correctlyEncodedQuestionUpdate = Form.Tree
    .QuestionWithKey(Form.Tree.tag(3), "label", "text", Nil)
    .asInstanceOf[Form.Tree]
    .asJson
    .noSpaces should beEqualTo(
    """{"id":3,"label":"label","text":"text","answers":[]}"""
  )
  val correctlyEncodedTextUpdate = Form.Tree
    .TextWithKey(Form.Tree.tag(4), "text")
    .asInstanceOf[Form.Tree]
    .asJson
    .noSpaces should beEqualTo("""{"id":4,"text":"text"}""")

  // Form tree decodings
  val correctlyDecodedGroup =
    jawn.decode[Form.Tree]("""{"children":[]}""") should beRight(
      Form.Tree.Group(Nil)
    )
  val correctlyDecodedQuestion = jawn.decode[Form.Tree](
    """{"label":"label","text":"text","answers":[]}"""
  ) should beRight(Form.Tree.Question("label", "text", Nil))
  val correctlyDecodedQuestionWithAnswers = jawn.decode[Form.Tree](
    """{"label":"label","text":"text","answers":[{"label":"label","value":1},{"label":"label"}]}"""
  ) should beRight(
    Form.Tree.Question(
      "label",
  ...
  val correctlyDecodedText =
    jawn.decode[Form.Tree]("""{"text":"text"}""") should beRight(
      Form.Tree.Text("text")
    )

  val correctlyDecodedGroupUpdate =
    jawn.decode[Form.Tree]("""{"id":2,"children":[]}""") should beRight(
      Form.Tree.GroupWithKey(Form.Tree.tag(2), Nil)
    )
  val correctlyDecodedQuestionUpdate = jawn.decode[Form.Tree](
    """{"id":3,"label":"label","text":"text","answers":[]}"""
  ) should beRight(
    Form.Tree.QuestionWithKey(Form.Tree.tag(3), "label", "text", Nil)
  )
  val correctlyDecodedTextUpdate =
    jawn.decode[Form.Tree]("""{"id":4,"text":"text"}""") should beRight(
      Form.Tree.TextWithKey(Form.Tree.tag(4), "text")
    )

  // Question answers encodings
  val correctlyEncodedFreeTextAnswer = Form.Tree.Question.Answer
    .FreeText("label".some)
    .asInstanceOf[Form.Tree.Question.Answer]
    .asJson
    .noSpaces should beEqualTo("""{"label":"label"}""")
  val correctlyEncodedNumericAnswer = Form.Tree.Question.Answer
    .Numeric(None, 1)
    .asInstanceOf[Form.Tree.Question.Answer]
    .asJson
    .noSpaces should beEqualTo("""{"label":null,"value":1}""")
  val correctlyEncodedFreeTextWithIdAnswer = Form.Tree.Question.Answer
    .FreeTextWithId(Form.Tree.Question.Answer.tag(2), "label".some)
    .asInstanceOf[Form.Tree.Question.Answer]
    .asJson
    .noSpaces should beEqualTo("""{"id":2,"label":"label"}""")
  val correctlyEncodedNumericWithIdAnswer = Form.Tree.Question.Answer
  ...
  // Question answers decodings
  val correctlyDecodedFreeTextAnswer = jawn.decode[Form.Tree.Question.Answer](
    """{"label":"label"}"""
  ) should beRight(Form.Tree.Question.Answer.FreeText("label".some))
  val correctlyDecodedNumericAnswer = jawn.decode[Form.Tree.Question.Answer](
    """{"label":null,"value":1}"""
  ) should beRight(Form.Tree.Question.Answer.Numeric(None, 1))
  val correctlyDecodedFreeTextWithIdAnswer =
  ...
  val correctlyDecodedNumericWithIdAnswer = jawn
    .decode[Form.Tree.Question.Answer]("""{"id":2,"value":1}""") should beRight(
    Form.Tree.Question.Answer
      .NumericWithId(Form.Tree.Question.Answer.tag(2), None, 1)
  )
