...
object CompanyContract {
  ...
  implicit val companyContractKindDecoder: Decoder[Kind] = Decoder[String].emap(_.toLowerCase match {
    case "unlimited" => Right(Kind.Unlimited)
    case "oneshot" => Right(Kind.OneShot)
    case other => Left(s"'$other' is not a member value of CompanyContract.Kind")
  })
